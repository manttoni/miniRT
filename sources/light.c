...
static int	in_the_shadow(t_vector collision, t_object *light, t_objarr *objarr)
{
	...
	shadow.direction = normalize_vector(v_sub(collision, light->location));
	shadow.start = v_sum(collision, v_mul(0.01, shadow.direction));
	shadow.distance = DBL_MAX;
	if (cast_ray(&shadow, objarr))
	{
		if (shadow.distance < v_dist(light->location, collision) - EPSILON)
			return (1);
	}
	return (0);
}
